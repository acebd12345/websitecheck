...
## 區塊 B：每站可覆寫欄位（對應 府內網站表 既有欄）

雲端逐站掃描時，從 `府內網站表`（`config.SITE_LIST_WS`）讀以下欄；此處只是**欄位語意對照表**，不重複存資料。
（`depth`/`save_html`/`pagination` 死欄已從母表移除，故不列。）
...
| `頁數更新日` | date | 頁數最後更新日 |
| `合規檢核` | 是/空 | 是否納入每月合規 AI 判讀集(取代舊 `web_check14站`,可增減) |
| `局處` | string | 分批/報表分組用;`局處Email` XLOOKUP 對照欄的對鍵 |
