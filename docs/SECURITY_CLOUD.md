...
| **Google Sheet ID** | `config.json.master_sheet_id`（`1dLiH7…`）＋ 分頁 gid | ✅ | env `MASTER_SHEET_ID`（非機密等級高，但不公開為宜） |
| **內部 AI 主機名** | `ai_base_url = https://<internal-ai-host>/v1` | ✅（僅存 private/config.json；已從 docs 抹除） | env `AI_BASE_URL`；勿寫入任何公開檔或 Sheet |
...
- 已從 `docs/ARCHITECTURE.md` 抹除內部主機名 `<internal-ai-host>`（commit 81bb762）。
...
5. **公開前再掃一次**：任何要 public 的 commit，先跑 `git ls-files | xargs grep` 掃 `<internal-ai-host>`/`@gov.taipei`/`AKIA`/`ghp_` 等 pattern（見 HERMES_HANDOFF 附指令）。
