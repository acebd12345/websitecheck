...
| `verify_suspicious.py` | 上層 | SUSPICIOUS 第二關**獨立工具**：讀既有 all_problems.csv → 對搶註候選送地端 AI 判 A/B/C。與 `full_overnight` 階段2 邏輯重疊，但保留其「不重爬、直接對舊 CSV 複查」的獨立用途 |
...
2. **SUSPICIOUS 第二關有兩份**：`engine/verify_suspicious.py` 與 `full_overnight.py` 的
   ...
   （verify_suspicious 保留是因它能「不重爬、直接對舊 CSV 複查」；若日後抽共用函式可消除漂移。）
