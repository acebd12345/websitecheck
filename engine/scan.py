...
合規檢核已搬至 engine/compliance.py(由 full_overnight worker 呼叫)。
...
用法:
  ...
  python -m engine.scan --profile health --sheet         (全量,讀 Sheet)
"""
import argparse
...
def main():
    ...
    ap.add_argument("--profile", choices=["health"], default="health")
    ...
    ap.add_argument("--workers", type=int, default=16, help="併發抓取數")
    ...
    print(f"載入 {len(sites)} 站 | 剖面=health | 來源={'Sheet' if args.sheet else 'CSV'} | 渲染={'開' if args.render else '標記'}\n", flush=True)
